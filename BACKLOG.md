...
- **Откуда берётся «Рекомендованая цена».** Пересчёт работает, но множителя в
...
- **Тумблер «Показывать цену»** прототипа не имеет. Заказчик подтвердил: пока
  только переключает своё состояние и шлёт `dealer:price-visibility`, ничего не
  скрывает. Что он должен скрывать на самом деле — открыто.
