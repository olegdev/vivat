...
function setStep(n, { scroll = true } = {}) {
  ...
  // Ниже `md` шаг — отдельный экран, и открывается он с верха. Иначе
  // прокрутка шага 0 остаётся, а документ без него короче: sticky-шапка
  // «Выбор магазина» уезжает за верхний край, и замок прокрутки не даёт
  // вернуть её обратно.
  if (scroll && isMobile()) window.scrollTo(0, 0);
