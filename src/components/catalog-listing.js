// =============================================================================
// CATALOG LISTING — сетка товаров, фильтры, сортировка, чипсы, URL
// =============================================================================
// Общая механика страницы каталога: покупательской (pages/customer/catalog.html)
// и дилерской (pages/dealer/catalog.html). Страницы отличаются только обвязкой —
// шапкой, подвалом, ценами и тумблером «Только модули», — поэтому всё, что ниже,
// живёт здесь в одном экземпляре, а страничные скрипты остаются проводкой.
//
// Разметка, с которой работает компонент, приходит из трёх партиалов:
// `catalog-settings` (воронка, таблетки, тумблер, сортировка, чипсы),
...
/**
 * @param {object}   opts
 * @param {Array}    opts.products  фикстуры товаров (в Blade — $products)
 * @param {Function} opts.rub       форматирование цены
 */
export function initCatalogListing({ products, rub }) {
  const grid = document.querySelector("[data-grid]");
  const tpl = document.querySelector("[data-card-tpl]");
  if (!grid || !tpl) return;

  // ===========================================================================
  // Grid render — clone the <template> per product (the future @foreach)
  // ===========================================================================
  function buildCard(p) {
    ...
    // На покупательских страницах прайс-режима нет и поле просто не читается.
    priceEl.dataset.priceBase = String(p.price);
    priceEl.dataset.priceRaw = priceEl.textContent;
  ...
  // Дилерский прайс-лист пересчитывает цены на каждую порцию свежей разметки:
  // сетка появляется позже, чем инициализируется шапка (см. price-mode.js).
  document.dispatchEvent(new CustomEvent("cards:rendered", { detail: { root: grid } }));
  ...
  // подмешиваем в то же состояние, что читается из формы.
  const modulesToggle = document.querySelector("[data-modules-only]");
  let modulesOnly = false;
  let currentSort = "popular";

  // Read the form into a plain state object: { collection:[…], facade:[…], …,
  // price_min, price_max, price, sort, modules_only }.
  function readState() {
    ...
    state.modules_only = modulesOnly;
    return state;
  }
  ...
  function matches(product, state, [min, max]) {
    ...
    if (state.modules_only && !product.isModule) return false;
  ...
  function toParams(state) {
    ...
    if (state.modules_only) params.set("modules_only", "1");
  ...
  // ===========================================================================
  // «Только модули» — дилерский тумблер панели настроек
  // ===========================================================================
  // Прототипа у него в макете нет; по решению заказчика это обычный фильтр —
  // такой же, как быстрые чипсы: переключение применяется сразу и уходит в URL
  // полем `modules_only` (см. BACKLOG).
  function syncModulesToggle() {
    if (modulesToggle) modulesToggle.setAttribute("aria-checked", String(modulesOnly));
  }
  if (modulesToggle) {
  ...
  function hydrateFromURL() {
    ...
    modulesOnly = params.get("modules_only") === "1";
    syncModulesToggle();
