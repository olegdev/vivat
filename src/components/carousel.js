...
export function buildCarouselSection({
  ...
  if (tabs?.length) {
    ...
    // Без описания верхнего отступа у ряда нет ни на одной ширине: на 1440 это
    // PDP (segments 922:103133 — 0/44/16), ниже md — тот же PDP (1997:315100:
    // 48 = 0 + 32 + 16, тогда как главная с описанием 56 = 8 + 32 + 16).
    if (!desc) {
      chips.classList.replace("pt-4", "pt-0");
      chips.classList.replace("max-md:pt-2", "max-md:pt-0");
    }
    // У ряда вкладок в макете ДВЕ высоты, и зависят они от страницы, а не от
    // наличия описания: 60 на обеих главных и обеих PDP (segments 878:103587,
    // 922:103133), 76 на каталоге, «Акциях» и «3D-моделях» (2248:97230,
    // 2338:254339).
    //
    // 60 — это НЕ 8/8. Мастер 632:31770 рисует 16 + 44 + 16 = 76, а экземпляр
    // на главной ужат руками до 60 (единственный оверрайд — `size`): верхняя
    // распорка исчезла из насчитанной раскладки, нижняя 766:30262 осталась —
    // 16 на y=44. То есть 0 сверху, чип 44, 16 снизу. Первая версия читала
    // `align=CENTER` мастера и делала 8/8 — центровка относится к тому, что
    // было до ужатия, а не к тому, что осталось. Ниже md ряд свой (8/32/16 на
    // главной) и здесь не трогается.
    if (compactChips) {
      chips.classList.replace("pt-4", "pt-0");
    }
    // Ниже md ряд — один из двух вариантов `segments`: `size=s` (56: 8 + чип
    // 32 + 16) там же, где на 1440 стоит ужатый ряд, `size=l` (68: 8 + чип
    // 44 + 16) у остальных. Высоту чипа по этому атрибуту даёт `.chip`.
    chips.dataset.chipsSize = compactChips ? "s" : "l";
