...
// него форма просто исчезала. Копия в окне при этом заявочная («Ваша заявка
// отправлена!»), см. BACKLOG. Подписка и сообщение об ошибке по-прежнему
// просто закрываются. Вход дополнительно открывает дилерский сеанс и уводит
// на дилерскую главную: это наше решение, а не макет.
import { signIn } from "./session.js";
import { setScrollLock } from "./scroll-lock.js";

// Окно, которое открывается вместо закрытого после успешной отправки.
const DONE = { "dealer-request": "dealer-success", director: "dealer-success" };
