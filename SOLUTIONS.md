...
## Ось варианта может существовать только на одной ширине

`quantity-stepper` переключает левую кнопку по своей оси `count`: единица — это
корзина (943:79876), два и больше — минус (2029:129546). На 1440 так и есть. На
360 **все** карточки заказа во всём файле — и дилерские, и покупательские —
рисуют минус рядом с единицей. Это не ошибка дизайнера в одном инстансе (первая
догадка, записанная в комментарий и прожившая до правок от клиента), а
отсутствие состояния на этой ширине: семь инстансов подряд не ошибаются.

**Считать состояние отсутствующим можно только пересчитав инстансы.** Один
противоречащий макет — вопрос; все до единого — правило.
