...
Обратная ловушка: **у узла `textDecoration` может не быть вовсе, а у стиля —
есть.** «очистить» в ящике фильтров (759:79196) своего поля не несёт, стиль
«Link XS underline» (604:23985) — несёт, и подчёркивание было снято «по узлу».
Индекс `fig.mjs` теперь разрешает стиль сам (`font.decoration`), а
`audit:type` сравнивает линию с `text-decoration-line` предков и ставит «✗д».
...
Там же всплыло, что «+N» палитры (`color-palette`, все четыре варианта)
подчёркнут — узлом, не стилем, 14/18 и 12/16. Поймал `✗д` на карточке сетки.
