...
**Если на узел наложен текстовый стиль, линию задаёт стиль, а поле узла
`textDecoration` протухает — как `fontSize`.** Ошиблись в обе стороны:
«очистить» (759:79196) — у узла поля нет, у стиля «Link XS underline» есть,
подчёркивание было снято; «+5» палитры (629:26709) — у узла UNDERLINE, у стиля
«Desktop/BodyS» линии нет, подчёркивание было добавлено на все карточки.
Индекс `fig.mjs` берёт `font.decoration` со стиля, а с узла — только когда
стиля нет (или он библиотечный и в файле отсутствует). `audit:type` сверяет
её с `text-decoration-line` предков («✗д») и учитывает стиль, переопределённый
в инстансе.
...
> **Обновление 15.09.** Геометрия ВСЁ-ТАКИ читается из `.fig`: `commandsBlob`
> — индекс в `message.blobs`, а блоб — команды пути `[op:u8][float32 × n]`:
> 0 Z, 1 M, 2 L, 3 Q, 4 C, координаты в пространстве VECTOR. `node
> scripts/fig-svg.mjs <symbol-id>` собирает SVG с заливками и сдвигами
> контейнеров; на pin 20 (2214:190771) вывод совпал с экспортом из Figma до
> третьего знака. Так выгружены воронка Thin, шеврон Thin и «more 16» для 360
> и Bold-воронка для 1440 — без MCP. Ниже — как было до этого.
...
## Относительная ссылка в общем партиале привязывает читателя к папке

Половины сайта лежат соседними папками (`pages/customer/`, `pages/dealer/`), и
относительный `href="main.html"` в общей шапке долго выглядел находкой: у
покупателя разрешается в покупательскую главную, у дилера — в дилерскую, одна
строка на обе половины. Ломается это в тот момент, когда появляется страница,
**общая для обеих половин, но лежащая в одной папке**. У нас это одиннадцать
контентных страниц раздела «Для бизнеса»: они физически в `dealer/`, а ссылки
на них есть и в покупательском подвале. Покупатель переходил туда — и дальше
уже не мог выйти: логотип вёл на дилерскую главную, корзина на дилерское
оформление, «Кухни» на дилерский каталог. Клиент увидел это как «некоторые
ссылки у кастомера открываются с дилерской плашкой».

Относительный адрес отвечает не на тот вопрос. Он спрашивает «в какой папке
лежит текущая страница», а нужно «кто её читает». Пока папка и читатель
совпадали, разница не проявлялась.

Лечится двумя частями, и обе нужны:

- **роль — в сеансе, а не в разметке.** Жёсткий `data-user` остаётся только на
  собственных экранах половины; общие страницы читают его из `localStorage`.
  Скрипт обязан быть **встроенным и синхронным сразу за `<body>`**: модуль
  страницы отложен, выполнится после разбора разметки, и чужая шапка успеет
  отрисоваться и мигнуть;
- **ссылка «в свою половину» пишется полным путём.** `../customer/main.html`
  плюс атрибут-маркер; у другой половины сегмент подменяется одной функцией.
  `../customer/…` и `../dealer/…` разрешаются одинаково из обеих папок, поэтому
  разметка остаётся одной на всех.

Признак, по которому это ловится заранее: если страница доступна из обеих
половин, ни одна относительная ссылка в её обвязке не может быть верной сама
по себе.

## Полоса у края — сначала проверить сам файл, потом вёрстку

Клиент увидел на широком экране чёрную полоску у правого края баннера и прочёл
её как «виден следующий слайд». Вёрстку это описывает правдоподобно: слайды
лежат стопкой, кадр упирается в ограничитель 1900, справа остаётся поле — легко
поверить, что медиа не дотягивается до края и сквозь неё что-то видно.

Проверка показала обратное. Прогон главной по всем ширинам от 1000 до 3200 с
сравнением прямоугольника медиа с прямоугольником кадра дал «перекрывает
целиком» на каждой — щели нет нигде и быть не может: кадр `overflow-hidden`, а
медиа при `object-fit: cover` всегда не меньше него.

Полоса оказалась внутри самого ролика: два крайних правых пикселя из 1920
тёмные во всех кадрах, по вертикали 420…1499. Баннер подгоняет ролик по ширине,
поэтому кайма попадает в кадр целиком и читается как элемент интерфейса.

Отсюда два приёма:

- **Пиксели ролика снимаются через `canvas`, но только по HTTP.** На `file://`
  `drawImage` из `<video>` пачкает холст, и `getImageData` бросает
  `SecurityError`. Поднять `python3 -m http.server` над `dist/` — и
  `videoWidth`, кадры по `currentTime` и профиль яркости крайних колонок
  читаются напрямую. Через скриншот то же самое не измерить: PNG ещё надо
  разобрать, а увеличение элемента до 4× ширины экрана роняет вкладку.
- **Срезается такая кайма растяжением от противоположного края**, а не
  `clip-path`: обрезка оставила бы прозрачную щель. `transform: scaleX(w/(w−n))`
  при `transform-origin: left` выталкивает n исходных пикселей за
  `overflow-hidden`. Работает, пока кадр подогнан по ширине; там, где он
  подогнан по высоте (мобильный баннер), правый край и так за кадром, и
  растяжение на 0.1% ничего не меняет.

Признак, по которому это ловится заранее: дефект «у края» не двигается при
смене ширины окна. Всё, что рождает вёрстка, — щели, наложения, переполнение —
от ширины зависит.

## Канвас `Notes` в том же файле — это наши аннотации, а не макет

Проверяя, откуда взялась подпись «Успейте купить любимые товары по специальной
цене» под «Акциями и скидками», поиск по экспорту нашёл её дважды — и обе
находки выглядели как подтверждение, что текст дизайнерский. На самом деле оба
узла лежат на канвасе **`Notes`**, а один из охватывающих фреймов там назван
буквально строкой нашего же комментария («Two canvases, one DOM: fluid below
`md`…»). То есть в файл залита наша собственная документация, и поиск по тексту
её не отличает.

Настоящий ответ дал `inst` по заголовочному блоку: `2395:105923` (и его
дилерский близнец `882:107968`) — 148 высотой, `text-container` 44, то есть
только строка заголовка. Описания в макете нет, подпись была нашей.

**Правило:** прежде чем считать текст найденным в макете, посмотрите, на каком
он канвасе. Канонический — `Design`; всё, что пришло с `Notes`, `Research`,
`Wireframes`, `presentation-*` и `trash`, доказательством не является. Проверять
надо не поиском по строке, а `inst` по тому блоку, который вы строите.

## Экземпляр спорит с мастером не только размером — ещё и высотой ряда

Три дефекта одной карточки оказались одной ошибкой: значения были сняты с
мастера, хотя каждый экземпляр говорит другое.

- **Ряд вкладок.** Комментарий в `carousel-section.html` ссылался на мастер
  `644:44577` — 16 + ряд 44 + 16 = 76. Но у дизайнера ДВЕ высоты, и зависят они
  от страницы: **60** на обеих главных и обеих PDP (`2395:105897`,
  `922:103133`), **76** на каталоге, «Акциях» и «3D-моделях» (`2248:97230`,
  `2338:254339`). Везде `padV=0, align=CENTER` при чипе 44, то есть 8/8 против
  16/16. Отсюда параметр `compactChips`.
- **Тег под названием.** Мастер `632:27865` рисует 26 с текстом 18. Все шесть
  карточек в `other-row 2395:105898` и все шесть в `758:57418` — 24 с текстом
  **16**. Значит `text-body-xs`, а не `text-[12px] leading-[18px]`.
- **Высота карточки.** В макете карточки ряда РАЗНОЙ высоты: 457 у той, чьё
  название в две строки, и 433 у остальных пяти; на 768 — 372 и 348. У нас
  название несло `h-12` (две строки всегда), а дорожка рельса растягивала
  карточки по самой высокой. Обе причины надо снимать вместе: без `items-start`
  на `[data-track]` снятие `h-12` ничего не меняет.

Ниже `md` фиксированная высота названия остаётся намеренно — так кнопки
«в корзину» стоят на одной линии; это решение клиента, а не недосмотр.

## Высота блока может сходиться при неверных кеглях

Блок соцсетей был центрирован в своей секции (`items-center`), тогда как макет
прижимает его книзу: `CN-container` во всех двенадцати экземплярах — распорка
сверху плюс сам блок, снизу ничего (96+220 на 1440, 64+220 на 768,
64+168+32 на 360). **Общая высота при этом сходилась**, поэтому дефект пережил
и сверку копии, и скриншоты, и `npm run audit:spacing` — тот увидел только
строку «в макете 96, у нас ничего» и заставил пойти посмотреть.

Внутри было то же самое: `Title` 60 = 28 + 32 без зазора, то есть H3 (24/28) и
Body L (20/32), а стояло H4 (20/24) + `gap-1` + Body N (16/24) = 52. Сумма
блока «почти сходилась» (212 при 220), и эти 8px выглядели мелочью — на деле
это были два неверных текстовых стиля.

**Правило:** сходящаяся высота контейнера ничего не доказывает про его
содержимое. Если внутренний блок отличается на 4–8px, это почти всегда кегль
или интерлиньяж, а не отступ, — и искать надо в `derivedTextData` размеров
текстовых боксов, а не в паддингах.

## Кегль узла в .fig врёт — дважды и по-разному

Строя `npm run audit:type`, первым же прогоном получил «заголовок 32/36 против
наших 30/36». Ни один токен не даёт 32, и узлов 32/36 в файле оказалось НОЛЬ,
тогда как 30/36 — 57 штук. Разбор узла `752:64204`:

```
fontSize        32              ← поле узла
lineHeight      100 PERCENT     ← то есть 32
size            453x36          ← насчитанная коробка
glyphs[0].fontSize   30         ← чем реально уложены буквы
textStyleId     69:3279         ← применённый текстовый стиль
```

Поля `fontSize` и `lineHeight` — это то, что лежало на узле ДО применения
текстового стиля; стиль их перебивает, а в файл они так и остаются. Правда
живёт в `derivedTextData`:

- **кегль** — `derivedTextData.glyphs[0].fontSize`;
- **интерлиньяж** — `layoutSize.y / baselines.length`, то есть коробка на число
  строк. **Не** `baselines[].lineHeight`: это natural leading шрифта, 38.25 там,
  где стиль говорит 36.

То же верно для экземпляров: у заголовка соцсетей мастер говорит 20, а
`glyphs[0].fontSize` в `derivedSymbolData` — 24. `slim()` в `fig.mjs` теперь
кладёт в индекс именно derived-значения.

**Но интерлиньяж из коробки — совет, а не приговор.** Коробку в макете случается
растянуть руками: «от» в карточке товара — узел 18x33 при кегле 16 (таких в
файле 503), тогда как стиль говорит 16/22. Поэтому `audit:type` считает дефектом
только кегль и вес, а расхождение интерлиньяжа помечает и оставляет человеку.

## Скрытый корень селектора даёт страницу ложных находок

`audit:type` на подвале выдал 32 расхождения подряд: «16/24 в макете против
12/16 у нас». Подвал при этом совпадал до пикселя — просто в разметке рядом
лежат десктопный и мобильный варианты, и `document.querySelector('footer')` на
1440 отдал мобильный: скрытый, но со своими вычисленными стилями. Обход внутрь
скрытое отбрасывал, а сам корень никто не проверял.

Селектор должен брать первый **видимый** узел из всех подходящих, а не первый
попавшийся. И признак этой ошибки узнаваем: расходится не одна строка, а все
сразу и в одну сторону.

## Свои фикстуры не сойдутся с чужими — сводить по порядку

Аудит копии и аудит кеглей паруют строки по тексту, и ровно те строки, ради
которых всё затевалось — цена, название товара, счётчики, — по тексту не
сойдутся никогда: в макете «148 320₽», у нас «60 000₽». После сведения по тексту
остатки с двух сторон надо зипнуть по порядку и пометить как недоказанную пару.
Именно так нашлась цена карточки каталога: 24/32 в макете против наших 24/28 —
тот же дефект, что до этого чинился в карточке рельса.

## Мастерская метрика — сигнал, а не приговор

`audit:type` берёт кегль из `derivedSymbolData`, но такая запись есть не у
каждого узла: туда попадает только то, что отличается от мастера. Значит для
остальных строк метрику приходится брать с мастера — и вот тут два случая,
которые нельзя путать.

- **Мастер верен.** У подвала три текстовых derived-записи на 32 строки:
  остальное совпадает с мастером, потому в derived и не попало. Выбросить эти
  строки — потерять весь блок.
- **Мастер — фантом.** Якорный ряд PDP (`Tab 914:103288`) держит в мастере
  собственные подписи «Фото» 24/28, а рендерятся вложенные `tab`-инстансы, и их
  derived говорит **16/24**. Прочитать мастер значит выдумать пять расхождений
  подряд.

Отличить их по данным нельзя, поэтому скрипт помечает такие строки «*» и **не
считает дефектом**: `✗` остаётся только там, где обе стороны твёрдые. Правило
общее и за пределами этого скрипта: если число пришло с мастера, оно годится
как повод пойти проверить, но не как основание что-то менять.

Попытка обойти это гейтом «есть ли у инстанса хоть одна derived-запись» не
работает: у подвала она есть, и подвал тут же осыпался с 32 строк до трёх.

## Ужатый экземпляр: «60» — это не «8/8»

Ряд вкладок на главной 60 высотой при чипе 44; мастер говорит `align=CENTER`.
Арифметика даёт 8 сверху и 8 снизу — и это неверно. Мастер `632:31770` рисует
`spacing 16` + ряд 44 + `spacing 16` = 76, а экземпляр `878:103587` **ужат
руками** до 60: единственный его оверрайд — `size`. В насчитанной раскладке
верхняя распорка исчезла, нижняя `766:30262` осталась на своём месте — 16 на
y=44. Реальная вертикаль: **0 сверху, чип 44, 16 снизу**. Клиент увидел 8
вместо 16 под чипами и был прав.

Тот же приём у мобильной PDP: `size=s` (56 = 8 + 32 + 16) ужат до 48 — и
снова пропала верхняя.

**Правило.** Если у экземпляра есть оверрайд `size`, его высота ничего не
говорит о распределении внутри. Смотреть надо `derivedSymbolData`: какие дети
остались и на каких `y`. Свойства стека мастера — `align`, `padV`, `gap` —
описывают то, что было до ужатия.

Как это читать быстро:

```
node scripts/fig.mjs raw <id>  →  symbolData.symbolOverrides   (есть ли size?)
                               →  derivedSymbolData[].size/transform.m12
```

Попутно: канвас `Notes` держит полную копию главной `2395:105837` (та, что в
документации значилась «Main на 1440»); настоящая — `878:103527` на `Design`.
Сейчас они совпадают посекционно, но ссылаться надо на `Design`.



## `fontSize` в снимке врёт чаще, чем арифметика кадра

Цена в карточке заказа стояла кеглем `h3` (24). В узле покупательской карточки
(943:34270) так и написано: `fontSize: 24`. А дизайнер нарисовал 20 — и виноват
не он.

Проверяется не чтением поля, а сложением родителя. Ряд цены 943:34268 шириной
181 состоит из «от» 18, зазора 8, цены 82, зазора 8 и старой цены 65. Если бы
цена была 24, она заняла бы 99 и ряд вышел бы 198. Дальше достаточно измерить
ту же строку своими токенами в браузере: `text-body-n` даёт ровно 65 (как
старая цена), «от» — 17 против 18, а 82 приходится на `text-h4`. Три совпадения
подряд — это шрифт, а не совпадение: наш Onest и Figma считают одинаково, и
значит 82 может быть только двадцатым кеглем.

То же самое всплыло в строке модуля: `fontSize: 20` при `size` 65×22, хотя 65 —
это шестнадцатый.

Отсюда правило: **у текста сверяются `derivedTextData.layoutSize` и ширина
родительского кадра, а `fontSize` — справочно.** Мерить свою сторону надо не
глазами, а `getBoundingClientRect()` у пробного `<span>` с нужным классом:
одной строки хватает, чтобы отличить 20 от 24.

Признак, по которому это ловится заранее: в разметке стоит кегль и тут же
`leading-*`, который его переопределяет. `text-h3 leading-6` — это след того,
что высоту подогнали под макет, а размер оставили чужой.

## Замок прокрутки должен быть общим и считать владельцев

Оверлеев на сайте девять — окна, бургер, меню каталога, поиск, ящик фильтров,
шторка модулей, шторка раздела, выбор города и полноэкранная карта. Каждый гасил
прокрутку сам: пять на `<body>`, три на `<html>`, один не гасил вовсе.

Ломается это на любой паре, открытой одна над другой. Окно поверх бургера:
закрылось окно — `classList.remove` снял класс, и страница поехала под всё ещё
открытым меню. Тот же сбой даёт `toggle(..., open)`, вызванный дважды подряд.

`components/scroll-lock.js` — множество владельцев и один класс на оба узла:

```js
setScrollLock("mobile-menu", open);   // идемпотентно
```

Страница стоит, пока в множестве есть хоть один ключ. Ключ — строка, своя у
каждого оверлея, поэтому повторный вызов с тем же значением ничего не меняет и
замок можно ставить там же, где `classList.toggle`.

Признак, по которому это ловится заранее: `overflow-hidden` дописывается руками
больше чем в одном файле.

## Ряд бывает выше своих кнопок — мерить надо ряд, а не текст

Клиент: «между нижним фото и следующим блоком у нас меньше, чем в макете».
Я померил от низа фото до верха кнопки «Описание», получил 32 и 32 — и
ответил, что расхождения нет. Мерил не то.

`Tab` в блоке характеристик — **59** высотой при вкладках **32** и
`align-items: center`. От фото до ряда действительно 32 (`spacing 32`), но до
самого ТЕКСТА — 32 + (59−32)/2 ≈ 45. Наш ряд облегал кнопку, то есть был 32:
текст поджимался к фото на 13, а весь блок выходил на 27 короче (392 против
411). На глаз это и видно как «слишком близко», хотя стык секций сходился.

**Правило.** Когда сверяешь отбивку, бери границу того же ящика, что и в
макете. Если фрейм задаёт себе высоту больше содержимого и центрирует его,
половина разницы — тоже отбивка, и в замере «до первого текста» она теряется.
Признак: у фрейма в Figma `height` больше, чем у любого его ребёнка, при
`align=CENTER`.

Проверять так:

```
node scripts/fig.mjs inst <tab-id>   → высоты вкладок (32)
node scripts/fig.mjs node <tab-id>   → высота самого ряда (59)
```

`audit:spacing` этого не ловит и не должен: он сводит секции верхнего уровня,
а ряд лежит внутри `Characteristics`.

## Четвёртый аудит: высоты ящиков и зазоры между строк

`npm run audit:box <page> <selector> <figma-id>` — то, чего не видели первые
три. `audit` сверяет копию, `audit:spacing` — стыки секций ВЕРХНЕГО уровня,
`audit:type` — кегли строк. Ящик внутри секции не сверял никто, и именно там
живёт «плывёт относительно макета»: ряд вкладок 59 против 32 не задел ни один
стык и ни одну строку.

Скрипт приводит обе стороны к плоскому списку ящиков в визуальном порядке и
парует по ВЫСОТЕ через LCS — та же машинка, что в `audit:spacing`, и по той же
причине: общих имён у .fig и разметки нет, а совпавший прогон высот держит
выравнивание. Геометрия берётся из `derivedSymbolData`, то есть из экземпляра;
мастерские размеры дали бы фантомы ровно так же, как в аудите кеглей.

Вторым блоком печатаются **зазоры между соседями внутри каждого родителя** —
высоты могут сойтись до пикселя, а разъедутся отбивки между строками, и в
списке высот этого не видно.

Чего он не умеет и не должен: карточка с нашей фикстурой даёт другие ширины и
другое дробление обёрток, поэтому строк «только в макете» / «только на
странице» много — читать надо совпавшие пары и одиночки рядом, а не итог.

Расхождения в 1px — норма: дробный интерлиньяж Onest даёт 158.5 там, где в
макете 158.

## Коробка кончается позже чернил — и обе стороны читают это по-разному

Один и тот же стык мы с другой машиной прочитали противоположно, и оба раза
сослались на кадр.

Зелёная полоса каталога начинается ровно там, где кончается **коробка** блока
`H2` (759:60482: H2 3864–4655, полоса с 4655) — значит отбивки нет. Но
последние 80 этой коробки — **хвостовая распорка внутри неё** (758:57415,
`spacing` на y=711). Чернила рельса кончаются на 4575, и на экране между
кнопкой «В раздел» и полосой ровно 80.

Решает не то, что написано в кадре, а **что несёт наша разметка**: наш рельс
это 178 + 76 + 457 = 711, хвостовой распорки в нём нет. Пока её нет внутри,
80 обязаны стоять снаружи.

**Правило.** Прежде чем снимать отбивку «потому что в макете её нет», проверь,
не лежит ли она внутри соседнего блока — и есть ли этот внутренний отступ у
нас. Сравнивать надо одинаковые ящики с двух сторон, а не «коробку там» с
«чернилами тут».

`npm run audit:spacing` сравнивает именно чернила и ловит это: он отрезает
ведущую и хвостовую `spacing` у секции. Прогон после такой правки показал бы
расхождение сразу.

## CSS `border` расширяет коробку, `strokeAlign: INSIDE` — нет

Клиент: «блок саммари больше, чем в макете, и иконка шеринга чуть смещена».
Одна причина на оба.

У сводки PDP (953:142727) есть обводка: `strokeWeight 1`, **`strokeAlign:
INSIDE`**, `#e7e7e7`. В Figma паддинг отсчитывается от края рамки, а обводка
ложится поверх него — коробка остаётся 412, содержимое 412 − 48 = **364**. В
CSS `border` при `box-sizing: border-box` съедает пиксель с каждой стороны:
контент выходит **362**, и всё внутри, включая прижатую вправо иконку, едет на
пиксель.

То же было на разделителе над ценой (`border-t` дал контейнер 133 вместо 132) и
в карточке отзыва (1686:63222, та же обводка INSIDE — содержимое 387 вместо
389).

**Правило.** Обводку `INSIDE` рисовать так, чтобы она не меняла коробку:
`ring-1 ring-inset ring-…` для всех сторон, `shadow-[inset_0_1px_0_…]` для
одной. `border` годится только там, где в макете обводка `OUTSIDE` или её нет
вовсе.

Проверять обводку так:

```
node scripts/fig.mjs raw <id>  →  strokeWeight / strokeAlign / strokePaints
```

`audit:box` теперь сам подсказывает: если пары совпали по ВЫСОТЕ, а по ширине
разошлись ровно на 2 — это он.

Заодно там же нашлись два обычных расхождения: у `Top` сводки нет нижнего
паддинга (коробка 244 кончается вровень с `geometry`, хотя `padV` в мастере
24), а между `Top` и `bottom` стоит 16, которых у нас не было. После всех
четырёх правок сводка сходится точно: 412×392 у дилера и 412×441 у покупателя.

## Класс, собранный строкой в рантайме, Tailwind не видит

Коралловое поле под картой — 72 на главных и 80 на PDP, значение приходит от
страницы. Первая версия делала `classList.replace("pb-[72px]", `pb-[${n}px]`)`
— и отступ обнулился: Tailwind генерирует только то, что написано в исходниках,
а склеенного в рантайме `pb-[80px]` в сборке нет. Класс снялся, новый ничего не
значит.

**Правило.** Значение, приходящее из JS, кладётся в **атрибут**, а правило
живёт в `app.css`:

```css
[data-stores-section][data-pad-bottom="80"] { padding-bottom: 80px; }
```

Так же безопаснее для Blade: сервер отрендерит атрибут, и поведение не зависит
от того, попал ли класс в сборку.

## Две карточки, похожие как близнецы, с разным зазором

`cards-other` и `cards-kitchen size=L` — оба 438 шириной, оба «картинка сверху,
две строки снизу», и оба на одной странице. Но `Info` у первого идёт с
**gap 0** (632:27854), у второго с **gap 4** (629:26978).

Сняв зазор во всех шаблонах разом — по находке в `cards-other`, — я сломал
карточку «Вся коллекция»: 445 вместо 450. Ловится только поимённой сверкой:
у какого шаблона какой мастер.

**Правило.** Прежде чем править «во всех похожих местах», выписать, какой
шаблон какой компонент рендерит. Похожесть на экране ничего не значит:
`data-pcard` это `cards-kitchen size=L`, `data-pcard-other-s` — `cards-other`,
и живут они в соседних рельсах одной страницы.

## Рамка, которая не расширяет коробку, но добавляет высоту

Пятый случай того же класса, и первый — по вертикали. Дилерская шапка выходила
**117 при 116**: `border-b` под верхней полосой делал её 45 вместо 44. В кадре
(мастер 607:27474) `top` ровно 44, а линия нарисована обводкой.

Правится так же, как боковые: `shadow-[inset_0_-1px_0_…]` вместо `border-b`.
Проверять стоит любой `border-b` на строке с заданной высотой — если высота
задана классом (`h-11`), рамка её не тронет, а если высота от содержимого,
добавит пиксель.

Подсказка `audit:box` про «ровно 2» ловит только горизонтальный случай;
вертикальный виден как расхождение высоты блока.

## Подсказка «ровно на 2» врёт, когда округлений два

В дилерской шапке она сработала на `row` 955↔953 и `logo-and-meny` 588↔586 —
и это НЕ рамка: меню складывается из «Каталог» 125 против 126 и группы 244
против 245, то есть два независимых округления ширины текста по пикселю.
Настоящая рамка была рядом, но по вертикали.

Читать подсказку надо как повод посмотреть, а не как диагноз: спуститься на
уровень ниже и увидеть, одна там разница в 2 или две по 1.

## Спросили про цвет — не трогай геометрию

Клиент спросил ровно одно: «#141414 · 60%?» — то есть усомнился в ЦВЕТЕ
обводки выбранного образца. Я полез в экспорт, увидел у узла `select` 28x28
с `strokeWeight 1 INSIDE`, решил, что и кольца никакого нет, и переписал
заодно геометрию: снял `ring-2 ring-offset-2`. Получилось тоньше и уже, чем
нарисовано, — сломал то, что было верным.

Правильное чтение лежало в соседнем комментарии того же файла: «коробка 36 на
кружке 28 — те же 2 и 2». Кольцо 2px с зазором 2 было правильным с самого
начала, ошибочным был только цвет.

**Правило.** Замечание про одно свойство — это замечание про одно свойство.
Менять рядом стоящее можно, только если под него есть отдельное доказательство;
«в экспорте я этого не вижу» доказательством не является, особенно когда
соседний слой в том же файле описывает обратное.

## `fontName` протухает так же, как `fontSize` — вес по экспорту не проверить

Я поменял вес крошек ниже `md` с 400 на 500, прочитав `fontName` мастера
(`1806:236770` → Medium). Живой файл говорит 400. У ТОГО ЖЕ узла в поле
`fontSize` стоит **16** при насчитанных **12** — то есть оба поля описывают
состояние до применения текстового стиля, и `fontName` ничуть не надёжнее
`fontSize`.

С кеглем спасает `derivedTextData.glyphs[0].fontSize`. **Для веса такого поля
нет**: `derivedSymbolData` начертание не несёт вовсе. Значит по экспорту вес
не проверяется в принципе — ни у мастера, ни у экземпляра.

Поэтому `audit:type` больше не считает расхождение веса дефектом: оно
помечается «вес» и идёт в подозрения, рядом с цветом и интерлиньяжем. Дефект
теперь только кегль — единственная метрика, у которой есть насчитанный
источник.

Практический вывод: если правка держится на весе из мастера — это не правка,
а догадка. Спрашивать или смотреть живой файл.

## Пустой `strokePaints` — это «не знаю», а не «нет обводки»

Я сверил обводку образца цвета, увидел `#141414 22%` и написал «совпадает
точно». Смотрел при этом на `color-frame` — нижний слой, общий для всех
состояний. У ВЫБРАННОГО образца поверх него лежит отдельный узел `select`
(759:78867) с `strokeWeight 1 INSIDE` и **пустым `strokePaints`**: краска в
снимок не попала, потому что приходит из стиля или переменной.

Пустой список красок при ненулевой толщине обводки значит «значение не
выгрузилось», а не «обводки нет». Если такой узел встретился — это ровно тот
случай, когда надо идти в живой файл или спрашивать, а не делать вывод по
соседнему слою.

Здесь верное значение — `#141414 60%`, то есть `border-alpha-pressed`; у нас
стояло кольцо 2px сплошного чёрного с отступом 2, заметно тяжелее и крупнее
нарисованного.

## `size-6 -m-1` — это окно 16 в холст 24, а не «иконка 24»

Клиент сказал «иконка отзывов будто размыта». Я прочитал кадр так:
`service-icons` **16x16**, внутри `container` **24x24 со сдвигом (-4,-4)** — и
решил, что слот 16, значит в разметке должно быть `size-4`. Стало мельче на
треть, и клиент увидел это сразу.

Решает геометрия самого рисунка: в `icon-comment.svg` глиф занимает
**3.0–18.5** внутри `viewBox 24`, то есть ~15.5 единиц. Кадр показывает холст
24 **в масштабе 1:1** через окно 16 со сдвигом -4 — в слот попадает почти весь
глиф, 15.5 из 16. А `size-4` вписывает весь холст 24 в 16, и глиф ужимается до
15.5 × 16/24 = **10.3**.

То есть `size-6 -m-1` воспроизводит кадр верно: 24 пикселя рисунка один к
одному, 16 в раскладке. Единственная неточность — CSS не обрезает вылезающее за
окно, но там прозрачный холст и около одной единицы глифа слева.

**Правило.** Прежде чем менять размер иконки, посмотреть границы пути внутри
`viewBox`. Слот 16 при холсте 24 не значит «рисовать в 16» — значит «окно 16 в
рисунок, нарисованный для 24». Признак — глиф, занимающий почти весь холст.

Побочно: `size-6` при `viewBox 24` даёт масштаб 1:1 и заведомо чёткие края, а
`size-4` — 0.667 и полупиксельные. То есть мельче и мутнее одновременно.


## Ширина в мастере — это ширина мастера, а не элемента (`fill-width=on`)

Планшетный бар CTA (`modal-button-container` 2483:246944 в экранном кадре
2483:246910) рисовался у нас кнопкой 190 у правого края, потому что в дереве
мастера `button-container` значится **188×44**, и это выглядело как «кнопка 188,
прижатая влево». На деле 188 — ширина САМОГО МАСТЕРА: у его варианта в имени
написано `fill-width=on`, а `raw` по узлу отдаёт
...
то есть кнопка тянется на всю область. Область `buttons` — 356, значит и кнопка
356. Проверяется это не размером (его `derivedSymbolData` для нерастянутого
ребёнка отдаёт `0×0`, то есть «не переопределял»), а **координатой подписи**:
текст лежит на 87.5 при ширине 181, а `(356 − 181) / 2 = 87.5` — он центрирован
в 356, а не в 213.

Правило общее: когда в мастере ширина ребёнка равна ширине мастера, это не
размер, а «fill». Смотреть надо `stackChildPrimaryGrow`, а не число.

И та же проверка сработала на соседе: `link-container` 144×32 в мастере, а в
инстансе — 356×48 с содержимым по центру (иконка 20 + зазор 4 + текст 84 = 108,
по 124 слака с боков). Обе половины бара **заполняются своими детьми целиком**;
144 и 188 не значат ничего.

## Координаты в `derivedSymbolData` — от РОДИТЕЛЯ, а не от корня

Ровно то, что делает проверку выше однозначной. В том же баре на 360:
`text-action` 129×44 @16,8 (поле бара), а внутри него `content` 74×44 @39.5 —
39.5, а не 55.5. Значит трансформация каждой записи считается от её
непосредственного родителя. Поэтому «подпись на 87.5» читается как «центр
внутри своего контейнера», и никакой другой ширины у контейнера быть не может.

Ещё одно следствие: у `raw <instance>` в `derivedSymbolData` **нет записи о
самом корне** — первая запись это его первый потомок. Если считать корень
первым, весь список съедет на один и любой вывод о размерах будет ложным.

## `flex-1` на элементе с собственными полями не делит пополам

Две области бара должны быть по 356 при внутренней ширине 720 и зазоре 8.
`flex-1` прямо на ссылке и на кнопке дал **340 и 372**. Причина не в контенте:
`flex-1` — это `flex: 1 1 0%`, но при `box-sizing: border-box` основа 0 у кнопки
упирается в её собственные поля (`btn-m` несёт `px-4`), то есть её минимум —
32, и свободные 712 − 32 делятся поровну поверх этого.

Лечится не подбором значений, а разделением ролей: **область делит ширину,
ребёнок её заполняет**.

```html
<div class="flex flex-1"><a class="w-full …">…</a></div>
<div class="flex flex-1"><button class="btn btn-m w-full …">…</button></div>
```

Это ровно то, что нарисовано: `text-action` / `buttons` — контейнеры без полей,
`link-container` / `button-container` — их содержимое во всю ширину. Совпадение
разметки с макетом здесь не случайность, а признак, что структура прочитана
верно.

## Иконка сверяется по имени варианта, а не по силуэту — `fig.mjs icons`

Печать и «поделиться» в строке «Выбрать все» стояли из кадра — но из
Bold-варианта, а в кадре Thin (2029:156849/156850): залитые кружки против
колец, обводка 2 против 1.5. Оба файла выглядели «как в макете», потому что
силуэт тот же; отличается ось `size=24 Thin | Bold` в имени символа.

Геометрию пути экспорт не отдаёт (`commandsBlob`), поэтому единственное, что
можно сверить локально, — какой именно символ набора `service-icons` стоит в
инстансе. Это и делает `node scripts/fig.mjs icons <id>`: идёт по дереву
мастера с наложением переопределений инстанса и печатает каждую иконку с её
осями — `pin · 16 · dark · 960:37188`. `npm run audit:icons` кладёт рядом
файлы `<img src>` блока через `docs/ICON-MAP.md` (файл → id символа); флаг `i`
в реестре включает это в `audit:all`.

Три вещи, без которых обход врёт:

- **`derivedSymbolData` — не список отрисованного.** Он перечисляет узлы с
  насчитанными данными; профиль в шапке планшета, у которого ни одного
  переопределения, там просто отсутствует. Перечислять надо детей МАСТЕРА,
  а derived оставить размерам.
- **`guidPath` переопределения — цепочка инстансов плюс узел.** Обычные фреймы
  по дороге в путь не входят: подмена иконки на pin в ссылке бара записана как
  `585:27839`, хотя между ссылкой и иконкой лежат `link-container › right-icon`.
  И читать переопределения надо с КАЖДОГО инстанса в цепочке — подмена лежит
  на ссылке, а не на баре.
- **Размер символа — не размер холста.** `size=16, name=pin` — это `container
  24x24 @-4,-4`: холст 24 в окне 16. Поэтому файл с `viewBox 0 0 24 24` и
  `size-6 -m-1` — правильная выгрузка этого символа, а не «иконка 24 вместо
  16» (см. урок про `size-6 -m-1`).

Скрытый слот иконки экспорт от показанного не отличает (SOLUTIONS › «Инстанс
молчит…»), так что у кнопки без иконки список всё равно назовёт обе стрелки
мастера — это «м», подозрение, не расхождение.

## Линия на одной стороне — `borderStrokeWeightsIndependent`, а не `strokeWeight`

Над извещением «Дилерская наценка…» в сводке ниже `xl` идёт линия 1px
#eeeeee — и в `tree` её не видно, потому что у узла `strokeWeight: 1` со
всех сторон, а рисуется только верх: `borderStrokeWeightsIndependent: true`,
`borderTopWeight: 1`, остальные стороны отсутствуют (= 0). То же устройство у
тапбара и карты в обратную сторону: `strokeWeight 0` при непустом
`strokePaints` — обводка есть в данных и не рисуется.

Поэтому индекс теперь несёт `stroke.sides` (`t/r/b/l` при независимых
сторонах), а `audit:box` в подсказке «цвет» считает обводку только с ненулевой
толщиной хотя бы по одной стороне и пишет, по какой (`(t)`). Рисуется такая
линия внутренней тенью (`shadow-[inset_0_1px_0_…]`), не `border-t`: INSIDE не
растит коробку, `border` растит (см. урок про INSIDE).

## `audit:box` сверяет и горизонталь, и цвет — но только у надёжных пар

Три дыры, закрытые 13.09: смещение от ЛЕВОГО края родителя (плашки на фото
сидели на 4 при 10 в кадре), ширина у пар одной высоты и цвет — заливка,
обводка, прозрачность. Все три считаются только у пар, чьи РОДИТЕЛИ тоже
образуют пару: иначе `phone` 128x24 шапки встаёт против нашего ряда 211x24 и
«разошёлся по ширине». Заливка сверяется в одну сторону — от кадра к странице,
и цвет ищется ещё и у детей нашего ящика: у нас он часто висит уровнем ниже
или выше, чем в Figma (образец красит кнопку, Figma — вложенный `color`). У
TEXT заливка — это цвет букв, его сверяет `audit:type`.

Что остаётся шумом и почему не убрано: хагающий контейнер против нашего
растянутого (`colors` 148 ↔ 364 при одинаковом содержимом слева), стухшая
коробка мастера (`button-container` 188 при `fill-width=on`), вложенность
разной глубины. Это «подозрения» — читать, а не чинить вслепую.

## Карта под листом: дорожка кончается над баром, а полёт целится в видимое

Три ловушки нижнего листа над картой, все три были в первой версии:

- **Дорожка листа не может доходить до нижнего края экрана, если там стоит
  что-то fixed.** На шаге 1 заказа секция была `inset-0`, а снизу лежали бар
  «Выберите дилера» и тапбар — 132px списка магазинов уезжали под них, и до
  последней карточки было не домотать. Во фрейме тапбара нет, а бар нарисован
  поверх листа, поэтому по макету это незаметно. Лечится не `padding-bottom`
  в списке, а границей дорожки: `order.js` ставит секции `bottom` по верхнему
  краю бара, там же, где меряет `top` по шапке. Читающие страницы этого не
  требуют: их карта на `z-50` накрывает тапбар целиком, как во фрейме.
- **`setCenter` целится в геометрический центр полотна, а видна только полоса
  над листом** — метка приезжала под лист. У ymaps 2.1 для этого есть поле:
  `map.margin.setDefaultMargin([0, 0, высотаЛиста, 0])` и
  `setCenter(ll, z, { useMapMargin: true })` — `flyTo()` в `stores-map.js`.
  Высоту отдаёт сам лист (`sheet.height()`), выше `md` она 0.
- **Тап по метке — это не «пользователь пошёл в карту».** Свёртывание листа по
  `pointerdown` на полотне исключает `.store-pin`: иначе выбор с метки сначала
  прятал список, в котором только что подсветилась карточка. Сама карточка
  при этом ставится ПЕРВОЙ строкой списка (`block: "start"`), потому что в
  свёрнутом листе видна одна-две строки и «nearest» оставлял её за краем.

И два общих правила карты в потоке страницы: колесо над ней не масштабирует
(`scrollZoom` выключен везде — для масштаба есть «+»/«−»), а ниже `md` в
потоке выключено и одноплечее перетаскивание — иначе свайп по карте
перехватывает прокрутку и на 320px карте «застреваешь». Двумя пальцами
(`multiTouch`) карта двигается по-прежнему; полноэкранная включает всё.
Промо-блок «Как добраться / Доехать на такси» появляется от 10-го зума и
`suppressMapOpenBlock` его не касается — спрятан по `[class*="-map-copyrights-promo"]`.

## Метрики шрифта: `audit:type` сверяет число строк и ширину уложенного текста

Кегль сходится, а блок на строку выше: Onest в браузере и в Figma укладывают
одну и ту же строку по-разному — на 328 при 22 px это ±26 px высоты, и ни
кегль, ни интерлиньяж этого не видят. Поэтому у пары ПО ТЕКСТУ (та же строка,
насчитанная укладка с обеих сторон) сравниваются ещё две цифры:

- **число строк** — `derivedTextData.baselines.length` против
  `round(высота Range / line-height)`; расхождение помечается `✗п` и считается
  переносом (раннер — в расхождения);
- **ширина однострочного текста** — `layoutSize.x` против ширины `Range`
  текстового узла (не элемента: у того паддинги и ширина контейнера); больше
  4 px и 3% — `шр`, мягкий сигнал: трекинг, начертание или не тот шрифт.

Обе цифры только насчитанные: с мастера их нет, и на мастерский филлер
(«Покупая товары для кухонь…» в полосе на 360) они не распространяются — там
пара идёт по порядку, а не по тексту.

## `url()` в `public/` резолвится — если писать его от корня

Заметка у иконок соцсетей в `app.css` говорила, что `url()` на файл из
`public/` Vite резолвить не умеет и предупреждает на каждой странице. Это
правда только для относительной записи: `url("../../assets/x.woff2")` Vite
ищет от файла со стилем, в `src/` его нет — отсюда «didn't resolve at build
time» 4×23 раза при сборке шрифтов. **Корневая запись `url("/assets/x.woff2")`
— другой путь в плагине**: Vite узнаёт файл из `publicDir`, молчит и при
`base: "./"` сам переписывает его в относительный от страницы (`../../assets/`
в `pages/*/`, `assets/` в корневом индексе). Так Onest лежит в
`public/assets/fonts/` и объявляется в `partials/fonts.html` одним `<style>`.

Двух вещей тот путь не проходит: hand-off вклеивает партиалы как есть — там
корневой `url()` переписывает сам `build-handoff.mjs` в `${up}assets/`; и
дев-сервер отдаёт его как есть, что и нужно. Иконки соцсетей трогать незачем:
рядом с `app.css` Vite их инлайнит в CSS, и запроса на них нет вовсе.

`<link rel="preload" as="font" crossorigin>` в клиентском билде не ставить: по
`file://` origin — `null`, CORS-запрос падает, и браузер качает файл второй раз
уже через `@font-face`. Один шум в консоли и нулевой выигрыш.

## Какой сборкой шрифта рендерил дизайнер — видно в `.fig`, файлы просить не надо

Сомнение «у дизайнера может быть другая сборка Onest, и глифы в строке сядут
иначе» закрывается без дизайнера. У каждого текстового узла в
`derivedTextData` лежит то, что Figma реально посчитала его шрифтом:

- `fontMetaData[].fontLineHeight` — (ascender + |descender|) / upm той сборки,
  которой рендерили. У Onest с Google это 1.275 = (970 + 305) / 1000;
- `baselines[].position.y` — базовая линия внутри строки. Figma кладёт её как
  `(lineHeight − fontLineHeight·fs) / 2 + ascender·fs`: для «- 10%» в бейдже
  (14 px в строке 18) выходит 13.655 — ровно то, что лежит в узле. На
  win-метриках (1212/334) было бы 15.146, и это не спутать.

Оба числа читаются `fig.mjs raw <text-id>`; метрики файла — из `hhea`/`OS/2`
(у Google-сборки hhea = typo = 970/−305, `USE_TYPO_METRICS` стоит, так что
браузер и Figma считают одно и то же). Совпало — значит, макет и
`public/assets/fonts/` — одна сборка, и расхождение бейджа по вертикали надо
искать в вёрстке, а не в шрифте. Не совпало бы — вот тогда просить файлы.

Оговорка про `fontSize`: у того узла поле говорит 20/32, а глифы уложены 14 в
строке 18 — считать надо по `glyphs[].fontSize` и `baselines[].lineHeight`,
см. «Кегль узла в .fig врёт».

## Буквы в пилюле: `audit:ink` — чернила против коробки

Коробка текста в бейдже стоит по центру и у нас, и в Figma (текст @5 в 32 —
и у нас line-box 22 от 5), базовая линия по одной формуле от одной сборки
шрифта: `(lh − 1.275·size)/2 + 0.97·size`. А цифры в L-бейдже сидели на 9
сверху / 11 снизу при 10.1 / 10.7 в макете: Chrome кладёт базовую линию на
целый пиксель, округляя ВНИЗ (21.32 → 21), Figma рисует дробную. Ни type,
ни box этого не видят — они сравнивают коробки. Видно только по чернилам.

`npm run audit:ink <page> <selector> [--scale 1,2,4] [--figma png --figma-scale 4]`
снимает элемент на каждой плотности и печатает зазор от края пилюли до
первого/последнего ряда чернил в CSS-px; с экспортом из Figma — то же для
него. Экспорты дизайнера лежат в `docs/figma-exports/` — единственный способ
получить настоящий растр макета без MCP.

Что из этого вышло: L-бейдж — поля 3/1 вместо 2/2, строка ниже на пиксель,
10/10 на 1x и 2x. S — макет ставит текст на 2.5 (6.5/7.5), Chrome умеет
только 7/7 (`pb-px`) или 6/8 (`pb-[2px]`): полупикселя нет, `pb-[1.5px]`
округляется как 2. Оставлен центр. Ширины шрифта тем же экспортом сошлись до
пикселя (53 против 53), так что версия «у дизайнера другая сборка Onest»
закрыта окончательно — метрики в `.fig` (fontLineHeight 1.275, базовая
13.655 при 14/18) совпадают с Google-сборкой.

## SVG в data-URI: сырой `#` обрывает картинку молча

`url("data:image/svg+xml,…fill=%22#292929%22…")` — всё после `#` браузер
считает якорем, SVG приходит без хвоста и не рисуется, в консоли пусто.
Галка в чекбоксе фильтров так и пропала: клетка выбранного состояния белая,
галки нет — «белое на белом». Цвет в data-URI пишется `%23292929`.
`audit:all` первым делом проверяет исходники на это и считает находку
расхождением.

## Крошка: точка живёт в своём ящике, и x надписи сверяется по тексту

В кадре шаг крошек — «текст + зазор + ящик `bullet` 6px», глиф точки в нём
прижат влево и уже ящика (16px: 5.6; мобильный 11px: 3.7). На 1440/768 шаг
76 = 62 + 8 + 6, между шагами 9; на 360 шаг 54 = 46 + 2 + 6, между шагами 0.
Ряд у нас плоский (`a • span`), поэтому ширина ящика — `w-1.5` на точке, а
зазоры — отрицательными полями под `gap-2` ряда. Без `w-1.5` «Акции» прилипает
к глифу на 2px левее — это было отгружено.

`audit:box` этого не видел: пары там строятся по высоте, а у плоского ряда с
вложенными шагами пар нет. Теперь он сверяет **x надписи от левого края
корня** по совпавшему тексту («✗ x надписи», твёрдо, допуск 1.5px) — только
для auto-width текста с настоящей копией (оверрайд или узел вне инстанса):
мастерская «Главная» в каждом шаге — заглушка, её x ничего не доказывает.


## Поток страницы: лишний блок и блок на 8px выше не видит ни один поблочный аудит

Блок соцсетей на 360 Акций стоял там, где кадр его не рисует, а рельс под ним
был 436 вместо 444. `audit:spacing` сверяет воздух на стыках — стыки были те же.
`audit:box` сверяет блок изнутри — внутри всё совпало. Разошлась СУММА.

`npm run audit:flow <page> <frame-id> --width W` берёт у кадра детей верхнего
уровня и требует, чтобы верх и низ каждого совпали (±1.5px) с краем
какого-нибудь блока страницы во всю ширину (≥85% окна), с учётом паддингов —
кадр рисует отступ отдельным `spacing`, мы паддингом. Печатается и первый
несошедшийся край: ниже всё обычно едет на ту же величину. `audit:all` гоняет
его на каждой строке СТРАНИЦА.

Два наблюдения, которые стоили времени:

- **Допуск 2px слишком широк**: низ последнего фото плитки (2077.5) «совпал» с
  низом сетки кадра (2075.6), и расхождение на 40px спряталось. Края
  блоков во всю ширину и 1.5px — рабочий компромисс с дробными 103.56 шапки.
- **Ряд рельса в Figma обнимает самую высокую карточку**, а не держит отступ:
  `other-row size=L` 444 = ряд 360 (третья карточка 360 при обычных 352–354) +
  скролл 32 + кнопка 52. Пустота под «Столешницы 31» — это чужая высокая
  карточка, и у ровных фикстур её нет. Воспроизводится `min-h` ряда.

## Подменённый вариант вложенного инстанса: аудит читал чужие кегли

Рельс «Модули» на PDP 360 (1997:315097) — мастер `catalog-row size=L`, в
котором карточки `cards-modul size=l` 320. Инстанс подменил их на `size=m`
160 (`overriddenSymbolID` → 1821:292059), и `audit:type` спускался в исходный
мастер: цена 18/26 против наших 14/20, все строки со «*», ничего не
засчитано — а настоящие отличия (кегль «+5», поля строки цены, зазор ярлыка
размера) прошли. Теперь `audit:type`, `audit:box` и `audit` берут символ из
`overriddenSymbolID` по тому же пути, что и derived. Правило общее: **у
вложенного инстанса сначала смотри подмену, потом `symbol`**.

Там же `✗д` «нашёл» подчёркивание у «+N» палитры, и оно было добавлено —
ошибочно: см. следующий абзац про стиль и поле узла.

## Иконка 16 — это отдельный символ, а не файл 24 в `size-4`

У набора `service-icons` на каждый размер свой символ со своей геометрией:
`info 16` занимает ~14 из 16, `info 24` в окне 16 — ~9. Поставить файл 24 в
`size-4` значит ужать рисунок на треть; `size-6 -m-1` с тем же файлом
сохраняет глиф, но это другой рисунок. Правильно — выгрузить нужный размер
(`node scripts/fig-svg.mjs <id>`). `audit:icons` сравнивает `viewBox` файла с
отрисованным размером и помечает уменьшение как «✗ холст 24 ужат до 16».

Там же три слепых пятна аудитов, закрытых 15.09:

- **интерлиньяж auto-width строки** — твёрдое «✗л»: у такой коробки нечего
  растягивать руками (цена модульной карточки 24/28 при 24/32 в кадре);
- **переопределённый стиль текста инстанса** (`styleIdForText` в
  overrides) — «Link S dotted» поверх Button S мастера; раньше линия
  читалась с мастера;
- **ширина корня** в `audit:box` — карточка отзыва 437 при 332 на 768 с той же
  высотой проходила молча.

И одна ложная тревога: вес по ширине глифа для цифр не определяется — в Onest
они моноширинные во всех начертаниях.


## Зачёркнутая цена: линию рисуем сами, по метрике шрифта

Высоту `line-through` в CSS не задать (`text-underline-offset` — только для
подчёркивания), а Chrome берёт её не из шрифта: у Onest `yStrikeoutPosition`
250/1000 — верх полосы на 0.25em над базовой линией, Chrome кладёт центр на
~0.31em, на 0.08em выше Figma (≈1px при 14, ≈2px при 24). Толщину он к тому же
округляет до 1 CSS px.

`.line-through` в `app.css` прячет родную линию прозрачным цветом (утилита
Tailwind задаёт только `text-decoration-line`) и рисует полосу фоном:
`background-size: 100% 0.05em`, `background-position-y: calc((1lh − 1.275em)/2
+ 0.97em − 0.25em)`. Формула верна для коробки высотой в строку — все
зачёркнутые цены проекта flex-элементы (проверено по семи страницам на двух
ширинах). У строчного `<span>` в обычном потоке фон покрывает только область
содержимого, и формула была бы `0.72em` — если такой появится, это место.
Замер на 8x: верх полосы 0.26em, толщина 0.05em на 16 и 12 px.

## «Посередине» — по видимому тексту, а не по коробкам строк

Подпись рубрики в компактной карточке 360 стояла ровно посередине между
названием и кнопкой по коробкам (8 и 8) — и одиннадцать раз читалась клиентом
как «снизу меньше». У текста 11/14 буквы начинаются на 3.5 ниже верха строки, а
подчёркивание выходит за её низ: по чернилам выходило 14.5 сверху и 7.5 снизу.
Когда клиент говорит «одинаково», мерить надо по пикселям — снимок на 4x и
первая/последняя тёмная строка между соседями (`scripts/ink.py` / скрипт в
истории 15.09), — и только потом двигать поля. Итог: поле строки 4, отступ
кнопки 6 — ≈10.5/11.5 по чернилам при карточке 243.

## Разметку с вариантами под сеанс не сохранять через textContent

Заголовок ящика фильтров несёт две подписи — «Фильтры» и «Все фильтры» — и
прячет лишнюю правилом `group-data-[user=dealer]`. `filters-panel.js`
запоминал его как `textContent` и так же возвращал при открытии: на «3D-моделях»
и «Декорах» получалось «Фильтры Все фильтры» одной строкой. Где JS временно
подменяет содержимое узла, исходник сохраняется и возвращается как
`innerHTML` — как это и было сделано в `catalog-listing.js`.

## Список классов в `swap()` — это копия разметки, и она протухает молча

`partials/stores.html` монтируется в четырёх режимах, и каждый режим снимает с
секции её «домашние» классы списком, написанным в `components/stores-map.js`:
`swap(section, ["bg-surface-accent", "pb-16", …], […])`. Когда у секции в
партиале поменяли `pb-16` на `pb-[72px]`, `classList.remove("pb-16")` стал
no-op — молча, без ошибки, — и под картой Контактов остались лишние 72:
блок выходил 752 вместо нарисованных 680, а «Наши сотрудники» уезжали вниз на
эти же 72 (`audit:flow` показывал ровно один сдвиг на всю страницу). Тот же
промах уже ловили в `enterSelectMode`, и там он стоил 8px на шаге 1 заказа.

Правило: класс в партиале, который кто-то снимает из JS, менять только вместе
с грепом по `src/components/` на старое написание. `classList.remove` никогда
не скажет, что снимать было нечего, поэтому ловится это не кодом, а
`audit:box` на корне блока («корень: макет 680, страница 752») либо
`audit:flow` — один и тот же сдвиг у всех блоков ниже.

## Значок у текста, чьё число строк зависит от ширины, ставит JS

Кадр рисует значок «info» на 2 ниже верха текста — для текста в две строки.
На широком телефоне и на планшете тот же текст помещается в одну, и те же 2
читаются как съехавший значок; клиент просил центрировать по строке, но при
двух строках — оставить как в кадре. CSS число строк не знает: ни `1lh`, ни
`align-items` не отличают одну строку от двух. `components/note-icon.js`
меряет высоту текста против его `line-height` и ставит `margin-top`: две и
больше — 2, одна — `(line-height − 16) / 2`; пересчёт на ресайз и через
`ResizeObserver` (заметка в скрытом шаге меряется нулём, пока её не покажут).
Разметке достаточно `data-note` на ряду, `data-note-icon` на значке и
`data-note-text` на тексте — у дилерского и покупательского текста в одном
слоте оба помечены, берётся видимый.

## Фокусное кольцо не гасят цветом — фокус уводят на контейнер

Бургер-меню при открытии ставило фокус на первую кнопку («Назад» / «×»), а
кольцо браузера прятали `outline-color: transparent` на `[data-mm-panel] :focus`.
В Chromium это работало, и проверка в Playwright ничего не находила; тестировщик
на телефоне видел синюю рамку на «×». Safari и Firefox **игнорируют
`outline-color` у `outline-style: auto`** — системное кольцо рисуется своим
цветом, что бы ни стояло в CSS.

Решение в двух частях, обе в `app.css` › `@layer base` и в скриптах оверлеев:

- Фокус при открытии — **на контейнер с `tabindex="-1"`** (`[data-mm-panel]`,
  `[data-modal-panel]`), а не на первый контрол. Скринридер попадает в диалог,
  ловушка Tab и возврат фокуса работают как прежде, а кольцо рисовать не на чем;
  Tab с контейнера уходит на первый контрол в DOM-порядке. Заодно на 360
  окно с формой больше не поднимает клавиатуру на открытии.
- Кольцо рисуется одно на весь сайт: `:focus:not(:focus-visible) { outline:
  none }` (тап и скрипт — без кольца) и `:focus-visible` — дизайновый
  `focus-ring` 1209:89930, 1px `#141414` на 3px. `[tabindex="-1"]:focus-visible`
  — `none`: контейнер после клавиатурного открытия тоже считается
  focus-visible, а рамка вокруг всей панели никому не нужна.

Проверять такое в Chromium бессмысленно; `npx playwright install webkit` на
этой машине падает (segfault на текущей macOS), поэтому проверка была по
механизму: ни один фокусируемый элемент не получает фокус от скрипта, а
правило для нефокус-visible — `outline: none`, у которого цвета нет.
