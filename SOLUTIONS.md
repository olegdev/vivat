...
## `size-6 -m-1` рисует 24 там, где в кадре 16

Клиент: «иконка отзывов будто размыта». Она и была — в полтора раза крупнее
слота.

В Figma иконка лежит так: `service-icons` **16x16**, а внутри него
`container` **24x24 со сдвигом (-4,-4)**. Это стандартный 24-й холст иконки,
обрезанный рамкой компонента. В разметке это повторили буквально —
`class="size-6 -m-1"`: бокс 24, отрицательные поля 4.

Но отрицательное поле меняет только РАСКЛАДКУ. Картинка по-прежнему красится
24x24, обрезки в CSS нет. В ряду высотой 18 она вылезала на 3 пикселя сверху и
снизу, а глиф выходил в 1.5 раза больше нарисованного — отсюда и ощущение
мутности: `viewBox` у SVG 24, при растяжении волосяные линии попадают на
половину пикселя.

**Правило.** Слот в кадре 16 — значит в разметке `size-4`. Внутренний 24-й
холст с отрицательным сдвигом воспроизводить не надо: `viewBox` сам
отмасштабирует рисунок. Признак ошибки — иконка, которая выше своего ряда.

Тот же приём остался ещё в одиннадцати местах (`grep 'size-6 -m-1'`), и в двух
из них сверху добавлен вертикальный сдвиг. Каждое требует своей сверки со
слотом в кадре — записано в BACKLOG.

Попутно: у всех наших SVG стоит `preserveAspectRatio="none"`. На квадратном
боксе это безвредно, но любой неквадратный слот исказит рисунок — это заряженное
ружьё.
